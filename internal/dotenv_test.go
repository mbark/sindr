...
func TestDotenv(t *testing.T) {
	t.Run("loads default .env file", func(t *testing.T) {
		...
		withMainStar(t, `
def test_action(ctx):
    # Use shmake.exec to create a .env file using Python
    shmake.exec('python3', '''
...
''')
    ...
    result = shmake.shell('echo $TEST_VAR')
    if result.stdout != 'hello':
        fail('expected "hello", got: ' + str(result.stdout))

shmake.cli(name="TestDotenv")
...
def test_action(ctx):
    ...
    result = shmake.shell('echo $EXISTING_VAR')
...
def test_action(ctx):
    # Verify existing variable is set
    result = shmake.shell('echo $OVERRIDE_VAR')
    if result.stdout != 'original':
        fail('initial env var not set correctly, got: ' + str(result.stdout))
...
''')
    ...
    result1 = shmake.shell('echo $FIRST_VAR')
