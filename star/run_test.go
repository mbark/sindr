...
shmake.command(name="test", action=test_action)
`)
		runStarlarkRuntime(t, "TestAsync")
	})
...
shmake.command(name="test", action=test_action)
`)
		runStarlarkRuntime(t, "TestRunTypeCreation")
	})
}
